...
DROP TRIGGER IF EXISTS `gew`.`projetos_BEFORE_INSERT`;

DELIMITER $$
USE `gew`$$
CREATE DEFINER = CURRENT_USER TRIGGER `gew`.`projetos_BEFORE_INSERT` BEFORE INSERT ON `projetos` FOR EACH ROW
BEGIN
